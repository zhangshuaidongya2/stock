#!/usr/bin/env python3
"""Build stock-by-day matrices from sina_data/today/*.csv.

Examples:
  python sina_flow/build_today_flow_matrix.py
  python sina_flow/build_today_flow_matrix.py --input-dir sina_data/today
  python sina_flow/build_today_flow_matrix.py --flow-output sina_data/flow.csv --price-output sina_data/price.csv --turnover-output sina_data/turnover.csv
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "整理 sina_data/today/ 下按 MMDD 命名的每日主力资金 CSV，"
...
def require_columns(df: pd.DataFrame, file_path: Path) -> None:
    ...
    if missing:
        raise SystemExit(f"文件缺少字段 {'、'.join(missing)}：{display_path(file_path)}")
...
def build_matrix(
    ...
    *,
    ...
    pivot_df = merged_df.pivot(index="代码", columns="日期", values=value_column)
