#!/usr/bin/env python3
"""Rank today's Sina fund-flow records by a supported field.

Examples:
  python sina_flow/rank_today_flow.py
  python sina_flow/rank_today_flow.py --top 20
  python sina_flow/rank_today_flow.py --bottom 20
  python sina_flow/rank_today_flow.py --date 0527
  python sina_flow/rank_today_flow.py --rank-by turnover --top 30 --format json
  python sina_flow/rank_today_flow.py --positive-only
  python sina_flow/rank_today_flow.py --max-change 5
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="读取新浪当日资金流 CSV，输出指定字段排行。"
    )
    parser.add_argument(
        "--input",
        help="输入 CSV 文件；不传则读取 sina_data/today/ 下 --date 对应的 MMDD.csv。",
    )
    parser.add_argument(
        "--date",
    ...
    parser.add_argument(
        ...
        default="main",
        help="排序字段：main=主力净流入，change=涨跌幅，turnover=换手率。",
    )
    parser.add_argument(
        "--positive-only",
        action="store_true",
